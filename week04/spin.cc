...
get_ticks()
{
 ...
 void processMenuEvents(int option)
 {
    switch (option)
    {
        ...
        case CLOCKWISE     : x = 0.0; y = 0.0; z = -1.0;         glutPostRedisplay(); break;
        case ANTICLOCKWISE : x = 0.0; y = 0.0; z = 1.0;          glutPostRedisplay(); break;
        case REVERSE       : x = 0.0; y = 0.0; z =- z;           glutPostRedisplay(); break;
        case FASTER        : speed = speed + 0.030f;             glutPostRedisplay(); break;
        case SLOWER        : speed = speed - 0.030f;             glutPostRedisplay(); break;
        case HALF          : speed = speed / 2;                  glutPostRedisplay(); break;
...
{
	int menu, option, option2, option3;

    option = glutCreateMenu(processMenuEvents);
    glutAddMenuEntry("Reverse", REVERSE);
    glutAddMenuEntry("Clockwise", CLOCKWISE);
    glutAddMenuEntry("Anti-Clockwise", ANTICLOCKWISE);

    option2 = glutCreateMenu(processMenuEvents);
    glutAddMenuEntry("Faster", FASTER);
    glutAddMenuEntry("Slower", SLOWER);
    glutAddMenuEntry("50% Slower", HALF);

    option3 = glutCreateMenu(processMenuEvents);
    ...
    glutAddSubMenu("Direction", option);
    glutAddSubMenu("Speed", option2);
    glutAddSubMenu("Colour", option3);
...
main(int argc, char *argv[])
{
    ...
    /* Specify buffer format, double buffered RGBA.  */
